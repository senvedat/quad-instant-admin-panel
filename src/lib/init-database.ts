...
CREATE TABLE IF NOT EXISTS saved_table_views (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(connection_id, table_name, created_by)
);
...
CREATE INDEX IF NOT EXISTS idx_saved_table_views_active ON saved_table_views(is_active);
...
INSERT INTO _admin_migrations (version) 
VALUES ('initial_v1.0.0')
ON CONFLICT DO NOTHING;
`;

export async function initializeAdminDatabase(): Promise<{ success: boolean; message: string }> {
  let pool;
  ...
  try {
    ...
    try {
      const migrationCheck = await pool.query(
        "SELECT COUNT(*) as count FROM _admin_migrations WHERE version = 'initial_v1.0.0'"
      );
      
      if (migrationCheck.rows[0].count > 0) {
        return {
          success: true,
          message: 'Admin database already initialized'
        };
      }
    } catch (error) {
    ...
    await pool.query(ADMIN_DB_SCHEMA);
    ...
    return {
      success: false,
      message: `Failed to initialize admin database: ${error instanceof Error ? error.message : 'Unknown error'}`
    };
  }
}
